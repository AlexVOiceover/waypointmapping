...
{
    /// <summary>
    /// Implementation of IWaypointService that delegates shape-specific processing to specialized services
    /// </summary>
    public class WaypointService : IWaypointService
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WaypointService"/> class.
        /// </summary>
        ...
        public WaypointService(
            IEnumerable<IShapeService> shapeServices,
            ILogger<WaypointService> logger
        )
        {
        ...
        )
        {
            _logger.LogInformation(
                "Legacy GenerateWaypointsAsync method called, converting to new format"
            );
        ...
        {
            _logger.LogInformation(
                "Generating waypoints for {ShapeCount} shapes",
                shapes?.Count ?? 0
            );
            ...
            foreach (var shape in shapes)
            {
                try
                {
                    var service = _shapeServices.FirstOrDefault(s =>
                        s.CanHandleShapeType(shape.Type)
                    );

                    if (service == null)
                    {
                        _logger.LogWarning(
                            "No service found to handle shape type: {ShapeType}",
                            shape.Type
                        );
                        continue;
                    }
                ...
                catch (Exception ex)
                {
                    _logger.LogError(
                        ex,
                        "Error generating waypoints for shape ID: {ShapeId}",
                        shape.Id
                    );
                }
            }
            ...
            _logger.LogInformation(
                "Generated {WaypointCount} waypoints in total",
                allWaypoints.Count
            );
