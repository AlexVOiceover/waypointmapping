...
namespace WaypointMapping.Server.Services
{
    ...
    public class RectangleShapeService : IShapeService
    {
        ...
        public RectangleShapeService(
            ...
            ILogger<RectangleShapeService> logger
        )
        {
        ...
        public List<Waypoint> GenerateWaypoints(ShapeData shape, WaypointParameters parameters)
        {
            _logger.LogInformation(
                "Generating rectangle waypoints with parameters: Altitude={Altitude}, Speed={Speed}, UseEndpointsOnly={UseEndpointsOnly}",
                parameters.Altitude,
                parameters.Speed,
            ...
            _logger.LogInformation(
                "Generating rectangle waypoints with {Count} coordinates",
                coordinates.Count
            );
            ...
            if (coordinates.Count < 4)
            {
                throw new ArgumentException(
                    "Rectangle shape requires at least 4 coordinate points"
                );
            }
            ...
            )
            {
                ...
                _logger.LogInformation(
                    "Camera calculations: Ground Width={Width}m, Height={Height}m, Line Spacing={LineSpacing}m, Photo Distance={PhotoDistance}m",
                    groundWidth,
                    groundHeight,
            ...
            if (parameters.IsNorthSouth)
            {
                ...
                double lngDistanceInDegrees =
                    effectiveLineSpacing / (111320.0 * Math.Cos(minLat * Math.PI / 180.0)); // Convert longitude to meters
                ...
                for (double lng = minLng; lng <= maxLng; lng += lngDistanceInDegrees)
                {
                    ...
                    else
                    {
                        ...
                        double columnHeightMeters = _geometryService.CalculateDistance(
                            minLat,
                            lng,
                            maxLat,
                            lng
                        ...
                        int numWaypoints = Math.Max(
                            2,
                            (int)(columnHeightMeters / distancePerPhoto) + 1
                        );

                        _logger.LogInformation(
                            "Column height: {Height}m, Photo interval: {Interval}s, Photo distance: {Distance}m, Creating {Count} waypoints",
                            columnHeightMeters,
            ...
            else
            {
                ...
                for (double lat = minLat; lat <= maxLat; lat += latDistanceInDegrees)
                {
                    ...
                    else
                    {
                        ...
                        double rowWidthMeters = _geometryService.CalculateDistance(
                            lat,
                            minLng,
                            lat,
                            maxLng
                        ...
                        int numWaypoints = Math.Max(
                            2,
                            (int)(rowWidthMeters / distancePerPhoto) + 1
                        );

                        _logger.LogInformation(
                            "Row width: {Width}m, Photo interval: {Interval}s, Photo distance: {Distance}m, Creating {Count} waypoints",
                            rowWidthMeters,
                            effectivePhotoInterval,
                            distancePerPhoto,
                            numWaypoints
            ...
            _logger.LogInformation(
                "Generated {Count} waypoints for rectangle with UseEndpointsOnly={UseEndpointsOnly}",
                waypoints.Count,
                parameters.UseEndpointsOnly
            );
            return waypoints;
