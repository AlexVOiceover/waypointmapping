...
namespace WaypointMapping.Server.Services
{
    ...
    public class PolylineShapeService : IShapeService
    {
        ...
        public PolylineShapeService(
            IGeometryService geometryService,
            ILogger<PolylineShapeService> logger
        )
        {
        ...
        public List<Waypoint> GenerateWaypoints(ShapeData shape, WaypointParameters parameters)
        {
            _logger.LogInformation(
                "Generating polyline waypoints with parameters: Altitude={Altitude}, Speed={Speed}, UseEndpointsOnly={UseEndpointsOnly}",
                parameters.Altitude,
                parameters.Speed,
            ...
            if (parameters.Speed <= 0)
            {
                ...
                _logger.LogWarning(
                    "Invalid speed value. Using default speed of {Speed} m/s",
                    parameters.Speed
                );
            }
            ...
            if (parameters.Altitude <= 0)
            {
                ...
                _logger.LogWarning(
                    "Invalid altitude value. Using default altitude of {Altitude} m",
                    parameters.Altitude
                );
            }
            ...
            if (parameters.LineSpacing <= 0)
            {
                ...
                _logger.LogWarning(
                    "Invalid line spacing value. Using default spacing of {LineSpacing} m",
            ...
            if (
                parameters.SensorWidth > 0
                && parameters.SensorHeight > 0
                && parameters.FocalLength > 0
            ...
            )
            {
                ...
                if (!parameters.ManualSpeedSet && parameters.PhotoInterval > 0)
                {
                    ...
                    _logger.LogInformation(
                        "Calculated optimal speed: {Speed} m/s based on photo interval",
                        calculatedSpeed
                    );
                }
                else
                ...
                _logger.LogInformation(
                    "Camera calculations: Ground Width={Width}m, Height={Height}m, Photo Distance={PhotoDistance}m",
                    groundWidth,
                    groundHeight,
                    distanceBetweenPhotos
                );
            }
            else if (effectivePhotoInterval <= 0)
            {
                ...
                _logger.LogInformation(
                    "Using default photo interval of {Interval} seconds",
                    effectivePhotoInterval
                );
            }
            ...
            _logger.LogInformation(
                "Polyline is {Type}",
                isClosedPolyline ? "closed (polygon)" : "open"
            );
            ...
            if (isClosedPolyline)
            {
                ...
                {
                    double fovH =
                        2 * Math.Atan(parameters.SensorWidth / (2 * parameters.FocalLength));
                    double groundWidth = 2 * parameters.Altitude * Math.Tan(fovH / 2);
                ...
                if (parameters.IsNorthSouth)
                {
                    ...
                    double lngDistanceInDegrees =
                        lineSpacing / (111320.0 * Math.Cos(minLat * Math.PI / 180.0));
                    ...
                    for (double lng = minLng; lng <= maxLng; lng += lngDistanceInDegrees)
                    {
                        ...
                        var intersections = FindPolygonIntersections(
                            coordinates,
                            boundingStartLat,
                            lng,
                            boundingEndLat,
                        ...
                        for (int i = 0; i < intersections.Count; i += 2)
                        {
                            ...
                            else
                            {
                                ...
                                double lineDistanceMeters = _geometryService.CalculateDistance(
                                    startLat,
                                    lng,
                                    endLat,
                                    lng
                                ...
                                int numWaypoints = Math.Max(
                                    2,
                                    (int)(lineDistanceMeters / distancePerPhoto) + 1
                                );
            ...
            else
            {
                ...
                for (int i = 0; i < coordinates.Count; i++)
                {
                    ...
                    if (i < coordinates.Count - 1 && !parameters.UseEndpointsOnly)
                    {
                        ...
                        {
                            additionalPoints = Math.Max(
                                1,
                                (int)(segmentDistance / distancePerPhoto) - 1
                            );
                        }
                        else
                        {
                            ...
                            additionalPoints = Math.Max(
                                1,
                                (int)(segmentDistance * minPointsPerMeter)
                            );
                        }

                        _logger.LogInformation(
                            "Segment {Index} length: {Length}m, Photo interval: {Interval}m, Adding {Count} intermediate points",
                            i,
                            segmentDistance,
                            distancePerPhoto,
                            additionalPoints
                        ...
                        for (int j = 1; j <= additionalPoints; j++)
                        {
                            ...
                            double lat =
                                coordinates[i].Lat
                                + fraction * (coordinates[i + 1].Lat - coordinates[i].Lat);
                            double lng =
                                coordinates[i].Lng
                                + fraction * (coordinates[i + 1].Lng - coordinates[i].Lng);
            ...
            _logger.LogInformation(
                "Generated {Count} waypoints for polyline with UseEndpointsOnly={UseEndpointsOnly}",
                waypoints.Count,
                parameters.UseEndpointsOnly
            );
        ...
        private bool DoesLineIntersectPolygon(
        ...
        )
        {
            ...
            // Special cases
            if (orientation1 == 0 && OnSegment(lat1, lng1, lat3, lng3, lat2, lng2))
                return true;
            if (orientation2 == 0 && OnSegment(lat1, lng1, lat4, lng4, lat2, lng2))
                return true;
            if (orientation3 == 0 && OnSegment(lat3, lng3, lat1, lng1, lat4, lng4))
                return true;
            if (orientation4 == 0 && OnSegment(lat3, lng3, lat2, lng2, lat4, lng4))
                return true;
        ...
        {
            return qLng <= Math.Max(pLng, rLng)
                && qLng >= Math.Min(pLng, rLng)
                && qLat <= Math.Max(pLat, rLat)
                && qLat >= Math.Min(pLat, rLat);
        }
        ...
        private List<Coordinate> FindPolygonIntersections(
            List<Coordinate> polygon,
            double lat1,
            double lng1,
            double lat2,
        ...
        )
        {
            ...
            for (int i = 0, j = polygon.Count - 1; i < polygon.Count; j = i++)
            {
                ...
                )
                {
                    ...
                    if (intersection != null)
                    {
                        ...
                        {
                            if (
                                Math.Abs(existing.Lat - intersection.Lat) < 1e-10
            ...
            {
                _logger.LogWarning(
                    "Odd number of polygon intersections found ({Count}). This may indicate an error in the intersection calculation.",
                    intersections.Count
                );
                ...
                if (startInside)
                {
                    ...
                    {
                        if (
                            Math.Abs(point.Lat - lat1) < 1e-10
                            && Math.Abs(point.Lng - lng1) < 1e-10
                        )
                        {
                            hasStart = true;
                            break;
                ...
                else if (endInside)
                {
                    ...
                    {
                        if (
                            Math.Abs(point.Lat - lat2) < 1e-10
                            && Math.Abs(point.Lng - lng2) < 1e-10
                        )
                        {
                            hasEnd = true;
                            break;
        ...
        private Coordinate? FindIntersection(
            double lat1,
            double lng1,
            double lat2,
            double lng2,
        ...
        )
        {
            ...
            if (Math.Abs(denom) < 1e-10)
            {
                ...
                )
                {
                    ...
                    {
                        Lat = (xPoints[1] + xPoints[2]) / 2,
                        Lng = (yPoints[1] + yPoints[2]) / 2
                    };
                }
